...
pub fn init() -> Result<ConnectionWithFullMutex> {
    ...
    let query = "CREATE TABLE IF NOT EXISTS iotstates (name TEXT, topic TEXT PRIMARY KEY, state BLOB)";
    connection
        .execute(query)
...
pub fn save_state(connection: &Connection, topic: &String, name: &String,  payload: &[u8]) -> Result<State> {
